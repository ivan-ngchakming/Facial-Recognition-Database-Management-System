    ...
    def init_models(self, prepare=True):
        ...
        for onnx_file in onnx_files:
            ...
            if model is None:
                ...
                del model
            elif model.taskname not in self.models and (self.allowed_modules is None or model.taskname in self.allowed_modules):
                logger.debug(f'find model: {onnx_file} {model.taskname} {model.input_shape} {model.input_mean} {model.input_std}')
                self.models[model.taskname] = model
            else:
                logger.debug(f'duplicated model task type, ignore: {onnx_file} {model.taskname}')
                del model
