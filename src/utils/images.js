...
export function getCornerBrightness(ctx, canvas) {
  var imageData = ctx.getImageData(canvas.width * .79, canvas.height * .01, canvas.width * .2, canvas.height * .2);
  var data = imageData.data;
  var r,g,b,avg;
  ...
  return Math.floor(colorSum / (canvas.width * .2 * canvas.height * .2));
}
